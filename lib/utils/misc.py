...
class CheckpointManager():
    def __init__(self, model, optimizer=None, ema_model=None, save_dir='', keep_num=1, rank=0, additions={}, mode='max'):
        self.model = model
        ...
        if self.rank == 0:
            ...
            self.metrics_fp.write('epoch,train_loss,test_loss,kd_loss,top1,top5\n')

    def update(self, epoch, metrics, score_key='top1'):
        print("i am here in metrics", metrics)
        if self.rank == 0:
            self.metrics_fp.write('{},{},{},{},{},{}\n'.format(epoch, metrics['train_loss'], metrics['test_loss'],metrics['kd_loss'], metrics['top1'], metrics['top5']))
