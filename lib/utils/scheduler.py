...
def build_scheduler(sched_type, optimizer, warmup_steps, warmup_lr, step_size, decay_rate, total_steps=-1, multiplier=1, steps_per_epoch=1, decay_by_epoch=True, min_lr=1e-5, multi_step_epochs=[25, 30, 35]):
    if sched_type == 'step':
        scheduler = StepLR(optimizer, step_size, gamma=decay_rate)
        decay_by_epoch = False
    elif sched_type== 'multi_step':
        scheduler = MultiStepLR(optimizer, milestones=multi_step_epochs, gamma=decay_rate)
        decay_by_epoch = False
