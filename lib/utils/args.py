...
class ParseKwargs(argparse.Action):
    def __call__(self, parser, namespace, values, option_string=None):
        ...
        for item in values:
            ...
            value = value.strip()
            # Parse numbers, bools, lists, dicts safely if possible
            try:
...
class ParseKwargsOld(argparse.Action):
    def __call__(self, parser, namespace, values, option_string=None):
...
#added by me for time series
parser.add_argument('--teacher-model-config', type=str, default='',
                    help='Path to teacher net config. Used for NAS model.')

#added for time series NEW
parser.add_argument('--input_modality', choices=["ts", "images"],  default="images",
                    help='Switch between input modality')
parser.add_argument('--val_size', default=None, type=float,
                    help='get this much of portion from train data for validation')
parser.add_argument('--chan_last', action='store_true',
                    help='If set: (B, T, C). Default: (B, C, T)')#if presents :(Batch, Seq_len, num_channels) deafault : (Batch, num_channels, Seq_len)
parser.add_argument('--data_base_folder', default='../ucr_data', type=str,
                    help='Base path to load time series dataset')
parser.add_argument('--n_chans', default=1, type=int,
                    help='#channels of each time series')
...
parser.add_argument('--downsample_size', default=100, type=int,
                    help='downsample each timeseries to this length if seq_len exceeds this value') 
parser.add_argument('--input_shape', default=(1,100), 
                    help='shape of each time series')
parser.add_argument('--ts_model_kwargs', nargs='*', action=ParseKwargs, default={},
                    help='store argument values to build ts model architectures')


#parameters added for diffusion training
parser.add_argument('--train_feature_space_diffsion', action='store_true', default=False,
                    help='Enable diffsuion model trianingo feature space')
parser.add_argument('--val_loss_monitor_mode', type=str, default='max',
                    help='Validation monitor metric to be min or max')
parser.add_argument('--val_loss_monitor_metric', type=str, default='top1',
                    help='Validation monitor metric')
parser.add_argument("--is_indexed", type=lambda x: str(x).lower() == "true", default=False)
parser.add_argument("--no-frozen_gen_prior", dest="frozen_gen_prior", action="store_false", default=True,
                    help="Unfreeze generative prior")
...
parser.add_argument('--kd_loss_weight', type=float, default=1.,
                    help='weight corrresponds to kd loss')
...
parser.add_argument('--experiment_root',default='exp_im_teachers',type=str,
                    help='Root directory for experiments (e.g., experimentsCifar100)')
...
parser.add_argument('--prefix_filename',default='',type=str,
                    help='file name prefix to save test results from different baselines(e.g., base, fits)')
...
parser.add_argument('--kd-loss-kwargs', nargs='*', action=ParseKwargs, default={})
parser.add_argument('--generative_prior_kwargs', nargs='*', action=ParseKwargs, default={})
...
#for limited dataset distillation
...
def parse_args():
    ...
    # Merge dict defaults from YAML with CLI overrides (CLI wins)
    for k, v in default_dicts.items():
        cli_v = args.__dict__.get(k, None)
        if cli_v is None:
            cli_v = {}
        v.update(cli_v)
        args.__dict__[k] = v
