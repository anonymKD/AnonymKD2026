...
def build_train_transforms_tinyimagenet(
    aa_config_str="rand-m7-mstd0.5",
    color_jitter=None,
    reprob=0.,
    remode='pixel',
    ...
    trans_l.extend([
        augment_ops.RandomResizedCropAndInterpolation(img_size, interpolation=interpolation),
        transforms.RandomHorizontalFlip()
    ])
    ...
    if aa_config_str is not None and aa_config_str != '':
        ...
        aa_params = dict(
            ...
            img_mean=tuple([round(x * 255) for x in mean]),
            interpolation=aa_interpolation
        )
        trans_l.append(augment_ops.rand_augment_transform(aa_config_str, aa_params))
    ...
    if reprob > 0:
        trans_r.append(
            augment_ops.RandomErasing(
                reprob, mode=remode, max_count=1, num_splits=0, device='cuda'
            )
        )
...
def build_val_transforms_tinyimagenet(interpolation='bilinear', mean=None, std=None):
    mean = IMAGENET_DEFAULT_MEAN if mean is None else mean
    std = IMAGENET_DEFAULT_STD if std is None else std
    ...
    trans_l = transforms.Compose([
        transforms.Resize(64, interpolation=interpolation),
...
def build_train_transforms_stl10(
    aa_config_str="rand-m7-mstd0.5",
    color_jitter=None,
    reprob=0.,
    remode='pixel',
...
def build_val_transforms_stl10(interpolation='bilinear', mean=None, std=None):
    mean = STL10_DEFAULT_MEAN if mean is None else mean
    std = STL10_DEFAULT_STD if std is None else std
    ...
    trans_l = transforms.Compose([
        transforms.Resize(96, interpolation=interpolation),
