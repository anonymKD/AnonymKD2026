...
from .mobilenet_v1 import MobileNetV1, MobileNetV1Tiny
from . import resnet
from .losses.gvd.gvd import LatentDiff
from lib.utils.layer_details import DIFF_MODULES


logger = logging.getLogger()

def build_model(args, model_name, pretrained=False, pretrained_ckpt='', model_config_file=None):
    #model_config_file set the default to args.model-config, this way one can pass seperate model-configs for teacher and student
    if model_config_file==None:
        model_config_file=args.model_config

    if model_name.lower().startswith('latentdiffusion'):
        teacher_channels = DIFF_MODULES[args.teacher_model]['channels']
        generative_prior_kwargs =args.generative_prior_kwargs
        ae_channels = generative_prior_kwargs.get('ae_channels', 1024)
        use_ae = generative_prior_kwargs.get('use_ae', True)
        inference_steps=generative_prior_kwargs.get('inference_steps', 5)
        num_train_timesteps=generative_prior_kwargs.get('num_train_timesteps', 1000)
        model=LatentDiff(teacher_channels=teacher_channels[0], use_ae=use_ae, ae_channels=ae_channels, inference_steps=inference_steps, num_train_timesteps=num_train_timesteps)
       
    elif model_name.lower() == 'nas_model':
    ...
    elif model_name.lower().startswith('ts'):
        from . import ts_models
        # model_name = model_name[3:]
        model = getattr(ts_models, model_name.lower())(args.n_chans, args.num_classes, args.ts_model_kwargs)

    elif model_name.lower() == 'mobilenet_v1':
        # mobilenet v1
        model = MobileNetV1(num_classes=args.num_classes)

    elif model_name.lower() == 'tiny_mobilenet_v1':
        # mobilenet v1
        model = MobileNetV1Tiny(num_classes=args.num_classes)
    ...
    elif model_name.startswith('tiny_tv_'):
        # build model using torchvision
        import torchvision
        import torch.nn as nn
        model = getattr(torchvision.models, model_name[8:])(pretrained=pretrained)
        # better stem for 64x64 images
